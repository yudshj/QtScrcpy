...
自定义按键映射操作方法如下：
- 选择已有脚本并点击“编辑脚本”，可在内置可视化编辑器中拖动按键位置、增删点击/拖动节点；也可以手写脚本放入 keymap 目录
- macOS 会把可编辑脚本放在 `~/Library/Application Support/QtScrcpy/keymap`，避免修改应用包后破坏代码签名；首次运行会自动复制内置脚本
- 启动投屏后打开编辑器，会自动使用当前解码画面作为底图；“截取当前画面”可随时刷新底图
- 保存后脚本会自动刷新、选中并应用到当前设备；“另存为”的文件需要放在 keymap 目录中
- 点击刷新脚本，确保脚本可以被检测到
- 选择需要的脚本
- 连接手机并启动服务之后，点击应用脚本
- 按键盘左上角的 `` ` / ~ ``（即脚本中定义的 switchKey）切换为自定义映射模式即可启用
- macOS 首次启用时需要在“系统设置 > 隐私与安全性 > 辅助功能”中允许 QtScrcpy；授权后再次按 `` ` / ~ `` 即可，不需要重启
- 再次按 `` ` / ~ `` 切换为正常控制模式
- 侧边栏的键盘图标用于显示或隐藏 keymap 可视化覆盖层，默认隐藏；也可以按 `Ctrl+K` 切换
- `gp-hyd.json` 是当前和平精英调校基线：`WASD` 控制方向摇杆，鼠标视角起点在屏幕中间偏上（`x=0.5, y=0.42`）
- 切换脚本、窗口失去焦点或断开设备时会自动退出自定义映射并释放仍按下的触点，避免角色持续移动或鼠标仍被捕获
