...
## 可视化按键映射工具
1. QtScrcpy 内置可视化编辑器：在主窗口选择脚本并点击“编辑脚本”，可以拖动映射点、编辑按键和注释、增删点击/拖动节点，并直接保存 JSON。启动投屏后，编辑器会自动使用当前解码画面作为底图，“截取当前画面”可刷新底图。保存后主窗口会刷新、选中并应用该脚本；“另存为”的文件需要放在 `keymap` 目录中。

   选择脚本并点击“应用脚本”后，视频窗口会加载当前脚本；侧边栏键盘图标用于显示或隐藏按键覆盖层，默认隐藏以避免遮挡游戏画面。切换脚本、窗口失去焦点或设备断开时会自动退出自定义映射并释放触点。
